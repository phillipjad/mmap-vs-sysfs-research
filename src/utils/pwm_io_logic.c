...
static bool is_pwm_exported(uint8_t chip, uint8_t channel) {
  ...
  (void)snprintf(pwm_direction_path, MAX_FILE_PATH_LENGTH,
                 "%s/pwmchip%u/pwm-%u:%u", PWM_CHIP_PATH, chip, chip, channel);
...
void init_pwm_channel(uint8_t chip, uint8_t channel) {
  if (!is_pwm_exported(chip, channel)) {
    ...
    (void)snprintf(pwm_channel_path, MAX_FILE_PATH_LENGTH,
                   "%s/pwmchip%u/pwm-%u:%u", PWM_CHIP_PATH, chip, chip,
                   channel);
...
void export_pwm_channel(uint8_t chip, uint8_t channel) {
  ...
  (void)snprintf(export_path, SYSFS_PWM_MAX_BUFFER_SIZE, "%s/pwmchip%u/export",
                 PWM_CHIP_PATH, chip);
...
void unexport_pwm_channel(uint8_t chip, uint8_t channel) {
  ...
  (void)snprintf(unexport_path, SYSFS_PWM_MAX_BUFFER_SIZE,
                 "%s/pwmchip%u/unexport", PWM_CHIP_PATH, chip);
...
void set_pwm_period(uint8_t chip, uint8_t channel, uint32_t period_ns) {
  ...
  (void)snprintf(path, MAX_FILE_PATH_LENGTH, "%s/pwmchip%u/pwm-%u:%u/period",
                 PWM_CHIP_PATH, chip, chip, channel);
...
void set_pwm_duty_cycle(uint8_t chip, uint8_t channel, uint32_t duty_ns) {
  char path[MAX_FILE_PATH_LENGTH + 1U] = {0};
  (void)snprintf(path, MAX_FILE_PATH_LENGTH,
                 "%s/pwmchip%u/pwm-%u:%u/duty_cycle", PWM_CHIP_PATH, chip, chip,
                 channel);
...
void enable_pwm(uint8_t chip, uint8_t channel, bool enable) {
  char path[MAX_FILE_PATH_LENGTH + 1U] = {0};
  (void)snprintf(path, MAX_FILE_PATH_LENGTH, "%s/pwmchip%u/pwm-%u:%u/enable",
                 PWM_CHIP_PATH, chip, chip, channel);
