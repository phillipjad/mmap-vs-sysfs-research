...
epwm_mmap_handle_t *epwm_mmap_init(uint8_t pwmss_instance, epwm_channel_t channel) {
	...
	static const uint32_t base_addrs[3U] = { PWMSS0_BASE_PHYS, PWMSS1_BASE_PHYS, PWMSS2_BASE_PHYS };
	...
	volatile uint8_t *map_base =
	    (volatile uint8_t *)mmap(NULL, EPWM_PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, (off_t)page_base);
	if (map_base == MAP_FAILED) {
		(void)close(fd);
		LOG_AND_EXIT("Failed to mmap PWMSS%u at 0x%08X", pwmss_instance, pwmss_base);
		return NULL;
	}
